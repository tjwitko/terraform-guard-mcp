...
const RESOURCE_CREDENTIAL_ATTR_RE =
  /(^|[\s{])(master_password|administrator_login_password|admin_password|password|client_secret|secret_key|access_key|private_key|api_?key|auth_token|token)\s*=\s*"([^"]*)"/gm;
...
export function scanResourceCredentials(source, file) {
  ...
  while ((block = RESOURCE_BLOCK_RE.exec(source))) {
    ...
    RESOURCE_CREDENTIAL_ATTR_RE.lastIndex = 0;
    let m;
    while ((m = RESOURCE_CREDENTIAL_ATTR_RE.exec(body))) {
      const [, , attr, value] = m;
