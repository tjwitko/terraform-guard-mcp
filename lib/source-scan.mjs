...
// HCL lets any string be written as a heredoc, and every rule in this file used to require a
// quoted value, so `secret_key = <<-EOT ... EOT` was invisible to all of them. gitleaks does not
// close the gap either: measured with identical high-entropy credentials, its generic-api-key rule
// fired on the quoted form and not on the heredoc, because that rule wants the secret adjacent to
// an assignment. Its private-key rule DOES match a PEM inside a heredoc, so multi-line key material
// is still covered by the other scanner -- the uncovered set is generic secrets, which is exactly
// what a password or token is.
//
// `<<TAG` and `<<-TAG` both parse; the terminator is a line containing only the tag. The value is
// trimmed, because `<<-` strips indentation and every heredoc ends with a newline that is not part
// of the secret.
function heredocAssignments(body, alternation) {
  const re = new RegExp(
    `(?:^|[\\s{])(${alternation})\\s*=\\s*<<-?([A-Za-z_][A-Za-z0-9_]*)[ \\t]*\\r?\\n([\\s\\S]*?)^[ \\t]*\\2[ \\t]*$`,
    "gm"
  );
  const out = [];
  let m;
  while ((m = re.exec(body))) out.push({ attr: m[1], value: m[3].trim() });
  return out;
}

function quotedAssignments(body, alternation) {
  const re = new RegExp(`(?:^|[\\s{])(${alternation})\\s*=\\s*"([^"]*)"`, "gm");
  const out = [];
  let m;
  while ((m = re.exec(body))) out.push({ attr: m[1], value: m[2] });
  return out;
}

/** Every literal assignment to one of `alternation`, written either way. */
function assignmentsIn(body, alternation) {
  return [...quotedAssignments(body, alternation), ...heredocAssignments(body, alternation)];
}

/** Heredoc bodies, for the rules that match on a value's shape rather than its attribute name. */
...
export function scanProviderCredentials(source, file) {
  ...
  while ((block = PROVIDER_BLOCK_RE.exec(source))) {
    ...
    for (const { attr, value } of assignmentsIn(body, [...CREDENTIAL_ATTRS, "credentials"].join("|"))) {
...
// Credential-shaped attributes on a *resource*, which is where the most obvious version of this
// defect actually lives and the one place this scanner did not look. Found by a generated project
// that wrote `password = "SecurePassword123!"` directly into an aws_db_instance: the provider-block
// rule reads provider blocks, the variable rule reads variable blocks, the tfvars rule reads
// .tfvars, and a literal master password in the resource itself fell between all three while the
// gate reported zero findings.
//
// Cross-cloud by attribute name, the same approach the provider-block rule takes: `password` and
// `master_password` (aws_db_instance, aws_rds_cluster), `administrator_login_password`
// (azurerm_mssql_server, azurerm_postgresql_server), `client_secret`, `private_key`, `token`.
const RESOURCE_BLOCK_RE = /(?:^|\n)\s*resource\s+"([\w-]+)"\s+"([\w-]+)"\s*\{/g;

const RESOURCE_CREDENTIAL_ATTRS =
  "master_password|administrator_login_password|admin_password|password|client_secret|secret_key|access_key|private_key|api_?key|auth_token|token";

// Attribute names that contain a credential word but hold a setting rather than a secret. A
// scanner that flags `password_policy` or `token_validity_units` produces findings nobody trusts,
// and this rule reads every resource in the configuration.
const RESOURCE_ATTR_NOT_CREDENTIAL = /(policy|length|reuse|validity|version|type|algorithm|rotation|enabled|arn|_id$|_name$)/i;

export function scanResourceCredentials(source, file) {
  const violations = [];
  RESOURCE_BLOCK_RE.lastIndex = 0;
  let block;
  while ((block = RESOURCE_BLOCK_RE.exec(source))) {
    const [, type, name] = block;
    const body = blockBodyAt(source, RESOURCE_BLOCK_RE.lastIndex - 1);
    for (const { attr, value } of assignmentsIn(body, RESOURCE_CREDENTIAL_ATTRS)) {
      if (RESOURCE_ATTR_NOT_CREDENTIAL.test(attr)) continue;
      // An interpolation is a reference to a variable or another resource, not a literal.
      if (value === "" || value.includes("${")) continue;

      violations.push({
        ruleId: "resource.hardcoded-credentials",
        severity: "critical",
        category: "secrets.hardcoded-credentials",
        provider: null,
        resourceType: type,
        // The file is part of the address now that the scan descends into child modules: a bare
        // `aws_db_instance.d` does not say whether it lives in main.tf or modules/db/main.tf.
        resourceAddress: `${type}.${name} in ${file} (source scan)`,
        attribute: attr,
        actualValue: "<redacted>",
        message:
          `${type}.${name} sets ${attr} to a literal value in source. It is committed to version ` +
          `control, and because it is not a variable it is the credential the deployment actually ` +
          `uses — not a placeholder someone will replace before applying`,
        remediation:
          `move it to a variable with no default, supplied at apply time from a secret store. For ` +
          `a managed database prefer the provider's generated-password option (on AWS, ` +
          `manage_master_user_password = true) so no password exists to store. Rotate this value: ` +
          `it is already in the repository's history`,
      });
    }
  }
  return violations;
...
// A connection string carrying its own password, matched on the *value* rather than the name.
// Every other credential rule here asks whether the attribute is called something credential-shaped;
// this one asks what the value is. A generated project wrote
//
//   database_url = "postgresql://log_admin:REPLACE_ME_PASSWORD@db_endpoint_placeholder/audit_logs"
//
// into a .tfvars file. `database_url` is not a credential-shaped name, so the name-based rule was
// silent, and the credential shipped in a file committed by default. It was pointed out in review
// and still shipped, which is the argument for a check rather than a comment.
//
// The placeholder does not make it safe, the same call identity-guard's connection-string rule
// makes: the deployment uses whatever is in this file, and a slot marked REPLACE_ME is one somebody
// fills in and commits. Provider-agnostic by construction -- it is a property of the URL, not of any
// cloud.
const CONNECTION_STRING_PASSWORD_RE = /\b[a-z][a-z0-9+.-]*:\/\/[^:/\s"']+:[^@/\s"']+@/g;

export function scanConnectionStrings(source, file) {
  const violations = [];
  const seen = new Set();
  // Quoted literals and heredoc bodies alike -- a URL written as a heredoc is the same credential.
  const candidates = [...[...source.matchAll(/"([^"]*)"/g)].map((m) => m[1]), ...heredocBodies(source)];
  for (const value of candidates) {
    if (!value || value.includes("${")) continue;
    CONNECTION_STRING_PASSWORD_RE.lastIndex = 0;
    if (!CONNECTION_STRING_PASSWORD_RE.test(value)) continue;
    const scheme = value.slice(0, value.indexOf("://"));
    if (seen.has(value)) continue;
    seen.add(value);
    violations.push({
      ruleId: "connection-string.embedded-password",
      severity: "critical",
      category: "secrets.hardcoded-credentials",
      provider: null,
      resourceType: null,
      ...
      actualValue: "<redacted>",
      message:
        `${file} contains a ${scheme} connection string with a password embedded in it. Terraform ` +
        `source and .tfvars files are committed by default, so this is a credential in version ` +
        `control — and it is the value the deployment uses, whether or not the password looks like ` +
        `a placeholder`,
      remediation:
        `build the URL at deploy time from its parts, taking the password from a secret store or a ` +
        `provider-generated secret, and keep the assembled string out of source. If this value has ` +
        `been committed, rotate the credential: removing the line does not remove it from history`,
    });
  }
  return violations;
}

// Directories whose .tf files are not the caller's to fix. `.terraform` holds modules Terraform
// downloaded from a registry: reporting a credential inside a third-party module is noise the
// caller cannot act on, and this project has already watched a model rewrite its own working files
// chasing errors that lived in vendored code. Same reason the Checkov integration drops vendored
// findings rather than passing them through.
// `.external_modules` is where Checkov puts modules it downloads with
// --download-external-modules, and it is inside the project rather than beside it. Omitting it
// here was not a small miss: a real run ended up with 5,040 vendored .tf files, every one of the
// 37 findings came from them and none from the caller's own code, and because the pre-commit hook
// runs this scan the commit became impossible to make. The model then started writing INTO the
// vendored directories trying to clear findings it could not own.
const SKIP_DIRS = new Set([".terraform", ".external_modules", ".git", "node_modules", ".venv"]);

// Child modules live in subdirectories, and this walk did not enter them: a `password` literal in
// modules/db/main.tf was invisible to every rule in this file. That is not a corner case -- putting
// resources in child modules is the convention Terraform documents and the one this repo's own
// benchmark fixture teaches. Paths are returned relative to the scan root so a finding names
// modules/db/main.tf rather than just main.tf.
export function terraformFilesUnder(dir, root = dir, acc = []) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      terraformFilesUnder(full, root, acc);
...
export function scanTerraformSources(dir) {
  ...
  const files = terraformFilesUnder(dir);
  ...
  for (const file of files) {
    ...
    if (file.endsWith(".tfvars")) {
      ...
      violations.push(...scanConnectionStrings(source, file));
      continue;
    }
    ...
    violations.push(...scanResourceCredentials(source, file));
    violations.push(...scanConnectionStrings(source, file));
